...
require_once e_BASE."languages/Bulgarian/lan_userclass2.php";
require_once e_BASE."languages/Bulgarian/lan_admin.php";
...
function check_allowed($class_id)
{
	...
	$row = $sql->db_Fetch();
	if (!check_class($row['userclass_editclass']))
	{
		header("location:".SITEURL);
		exit;
	}
...
if (isset($_POST['disp']))
{
	...
	if ($sql->db_Select('users', 'userid, username', "user_class = '{$class_id}' OR user_class REGEXP('^{$class_id},') OR user_class REGEXP(',{$class_id},') OR user_class REGEXP(',{$class_id}$') ORDER BY userid"))
	{
		while ($row = $sql->db_Fetch())
		{
			$text .= "
		<tr>
			<td class='forumheader3'><a href='".SITEURL."user.php?id.".$row['userid']."' alt=''>".$row['userid']."</a></td>
			<td class='forumheader3'>".$row['username']."</td>
		</tr>";
		}
	}
	$text .= "
...
if (strstr(e_QUERY, 'clear'))
{
	...
	$class_id = $tmp[1];
	check_allowed($class_id);
	if ($sql->db_Select('users', 'userid, user_class', "user_class = '{$class_id}' OR user_class REGEXP('^{$class_id},') OR user_class REGEXP(',{$class_id},') OR user_class REGEXP(',{$class_id}$')"))
	{
		while ($row = $sql->db_Fetch())
		{
			$uidList[$row['userid']] = $row['user_class'];
		}
...
elseif(e_QUERY)
{
	...
	unset($uidList);
	if ($sql->db_Select('users', 'userid, user_class', "userid IN({$tmp2[1]})"))
	{
		while ($row = $sql->db_Fetch())
		{
			$uidList[$row['userid']] = $row['user_class'];
...
if (isset($_POST['createclass']))
{
	if($_POST['userclass_name'])
	{
		...
		if (false !== $editclass_check || check_class($editclass))
		{
			$i = 1;
...
$userclass_name = varset($userclass_name,'');
$userclass_description = varset($userclass_description,'');
...
if(isset($_POST['edit']))
{
	$sql->db_Select("users", "userid, username, user_class", "banned != 1 ORDER BY username ");
	$c = 0;
	$d = 0;
	while ($row = $sql->db_Fetch())
	{
		...
		{
			$in_userid[$c] = $userid;
			$in_username[$c] = $username;
//			$in_userlogin[$c] = $user_login ? "(".$user_login.")" : "";
			$in_userlogin[$c] = $username;
			$c++;
		}
		else
		{
			$out_userid[$d] = $userid;
			$out_username[$d] = $username;
//			$out_userlogin[$d] = $user_login ? "(".$user_login.")" : "";
			$out_userlogin[$d] = $username;
			$d++;
		}
	}

	$text .= "<br /><table class='fborder' width='90%'>
		<tr>
...
{
	$sql_tmp = new db;
	$class_count = $sql_tmp->db_Count("users", "(*)", "WHERE user_class = '{$p_class_list}' OR user_class REGEXP('^{$p_class_list},') OR user_class REGEXP(',{$p_class_list},') OR user_class REGEXP(',{$p_class_list}$')");
	return $class_count;
}
