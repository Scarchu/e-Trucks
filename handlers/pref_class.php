...
class prefs 
{
	...
	var $DefaultRows = "name='e107' OR name='menu_pref' OR name='notify_prefs'";
	...
	function ExtractPrefs($RowList = "", $use_default = FALSE) 
	{
		...
		{
			$this->prefVals['core'][$row['name']] = $row['value'];
		}
		return TRUE;
	}
	...
	*/
	function get($Name) 
	{
		...
		if($get_sql->db_Select('core', '*', "`name` = '{$Name}'", 'default')) 
		{
			$row = $get_sql->db_Fetch();
			$this->prefVals['core'][$Name] = $row['value'];
	...
	*/
	function set($val, $name = "", $table = "core", $uid = USERID) {
		...
		switch ($table ) {
			case 'core':
			if(!$sql->db_Update($table, "value='$val' WHERE name='$name'"))
			{
