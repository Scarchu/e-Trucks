...
class e_emotefilter {
	...
	function e_emotefilter() /* constructor */
	{
		...
		//	save_prefs();
		//}
		$this->emotes = $sysprefs->getArray("emote_Skype");		//getArray("emote_".pref['emotepack'])
			...
			foreach($this->emotes as $key => $value)
			{
			  ...
			  {	// Only 'activate' emote if there's a substitution string set
				...
				$key = str_replace("!", "_", $key);
				  ...
				  $filename = e_BASE."uploads/emotes/Skype/" . $key;	//towa si e taka
				  $fileloc = SITEURLBASE.e_UPLOADS_ABS."emotes/Skype/" . $key;		//triabwa da se proweri tocno kakwo stawa tuk???
//echo $fileloc;
